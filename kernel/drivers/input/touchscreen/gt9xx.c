...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    u8 cfg_info_groups[][GTP_CONFIG_MAX_LENGTH] = {CTP_CFG_GROUPS};
    {
    ...
    if(i < 0) {
        printk("GT9xx no config found for %02x %02x\n", bid, cid);
        return -1;
    }
    printk("GT9xx config%d found for %02x %02x\n", i, bid, cid);
    swap_xy = cfg_info_groups[i][2] & 0x10;
    ts->gtp_cfg_len = cfg_info_groups[i][3];
    memset(&config[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
    memcpy(&config[GTP_ADDR_LENGTH], cfg_info_groups[i]+4, ts->gtp_cfg_len);
