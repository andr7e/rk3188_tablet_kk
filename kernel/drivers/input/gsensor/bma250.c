...
#define BMA250_BW_SET			2		// elf, bma222
		...
		const struct i2c_device_id *id)
{
	...
	if (client->addr == BMA222_ADDR) {
		return -1;
	}
	client->addr = BMA222_ADDR;

	tempvalue = i2c_smbus_read_word_data(client, BMA222_CHIP_ID_REG);
	if ((tempvalue&0x00FF) != BMA222_CHIP_ID) {
		printk(KERN_INFO "BMA250 & BM222 Device not fount! id:0x%x\n",tempvalue);
		return -1;
	}
	printk(KERN_INFO "Find BMA222 Device!!!\n");
		...
		const struct i2c_device_id *id)
{
	...
	else if (0 != bma222_probe(client, id)) {	
